...
- `reset_key_hold`: default to `0.05` seconds for reliable `i` input.
...
node will load it independently from `key_hold`.

## Testing

Update the reset-command unit test first so it expects a 50 ms hold for `i`
and 10 ms holds for every `q`. Confirm that this test fails before changing
production code, then run the complete `vehicle_control` test suite.
