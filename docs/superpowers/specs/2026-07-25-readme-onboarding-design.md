...
- 결론과 실행 명령을 먼저 제시하고 배경 설명은 짧게 쓴다.
- 긴 문단보다 표, 목록, Mermaid와 짧은 코드 블록을 사용한다.
- 루트 README에는 온보딩에 필요한 핵심값만 두고 세부 내용은 패키지 README로
  연결한다.
- 같은 파라미터 표와 문제 진단 절차를 여러 README에 반복하지 않는다.
- 기존 패키지 README는 전면 확장하지 않고, 실제 구현과 다른 내용과 중요한
  누락만 고친다.
