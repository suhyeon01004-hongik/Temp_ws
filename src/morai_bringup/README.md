...
GPS localization, path manager, Pure Pursuit/Stanley 선택형 횡제어기와
PID·UDP 제어 송신기의
...
| `molit_2026_autonomous.launch` | 자율 경로 추종 stack | stack, Competition Status 수신, 선택형 횡제어/PID, UDP 제어 송신기 |
...
다음 세 config 경로를 추가한다.

| 인자 | 기본값 | 역할 |
| --- | --- | --- |
| `vehicle_status_config` | `morai_udp_bridge/config/molit_2026_vehicle_status.yaml` | Competition Status UDP port·topic·stale 설정 |
| `controller_config` | `morai_path_tracking/config/molit_2026_path_tracking.yaml` | 횡제어기 선택, Pure Pursuit/Stanley/PID 게인, 목표 속도, 안전 임계값 |
| `control_sender_config` | `morai_udp_bridge/config/molit_2026_control.yaml` | `/control/actuator_command` UDP 송신과 watchdog 설정 |

기본 직선 목표 속도는 `58.0 km/h`다. PID 피드백은
`/vehicle/competition_status.velocity_x_mps`이고 odometry는 pose에만 사용한다.
MORAI Network Settings의 Competition Vehicle Status Destination Port를
`9094`로 맞춰야 한다.
...
| Competition Status UDP·topic | `molit_2026_vehicle_status.yaml` | `morai_udp_bridge` |
| GPS CRS·map offset, yaw 보정·동기화 | `molit_2026_kcity.yaml` | `morai_localization` |
| 전역경로·전방 100 m local path 정책 | `molit_2026_kcity_route_path.yaml` | `morai_path_manager` |
| 횡제어기 선택·제어 게인·목표 속도·안전 임계값 | `molit_2026_path_tracking.yaml` | `morai_path_tracking` |
| 자율 제어 UDP 송신·watchdog·optional gear | `molit_2026_control.yaml` | `morai_udp_bridge` |
...
/localization/pose
  -> /localization/odometry (제어용 위치·자세)
...
MORAI Competition Vehicle Status UDP :9094
  -> /vehicle/competition_status (vehicle x velocity m/s, gear)

/localization/odometry (pose) + /local_path + /vehicle/competition_status
  -> Pure Pursuit 또는 Stanley / PID
...
- 현재 localization pose는 GPS X/Y와 IMU yaw를 직접 결합한다. odometry
  ...
  이 추정 속도는 PID feedback으로 쓰지 않는다. noise를 켤 때는 pose fusion
  구현을 EKF/UKF 기반으로 교체해야 한다.
- 자율 경로 추종은 GPS와 IMU sensor noise가 꺼진 상태를 전제로 한다.
- 목표 속도와 Pure Pursuit/Stanley/PID 게인은
  `molit_2026_path_tracking.yaml`에서 튜닝한다.
